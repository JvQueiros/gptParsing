...
docker run -d --name listener --network gpt_parsing_default -p 8000:8000 asasgroup/garantias_ia:latest
docker run -d --name celery_worker1 --network gpt_parsing_default asasgroup/garantias_ia:latest celery -A tasks worker --loglevel=info